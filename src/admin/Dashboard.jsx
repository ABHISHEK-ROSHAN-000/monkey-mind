    ...
    <>
      ...
      <div className="card">
        ...
        <p style={{ color: 'var(--muted)', fontSize: '.9rem' }}>
          Firebase: {isFirebaseConfigured ? 'configured ✓' : 'placeholder mode — set VITE_FIREBASE_* (see .env.example)'}<br />
          Cloudinary: {isCloudinaryConfigured ? 'configured ✓' : 'placeholder mode — set VITE_CLOUDINARY_*'}
        </p>
        <div className="row">
          <Link className="btn" to="/admin/projects">Manage projects</Link>
          <button className="btn ghost" onClick={() => { if (confirm('Reset all CMS data to placeholders?')) s.resetAll(); }}>Reset demo data</button>
        </div>
      </div>
      <div className="card">
        <b>Recent projects</b>
        <table className="tbl">
          ...
          <tbody>
            {[...s.projects].sort((a, b) => b.order - a.order).slice(0, 5).map((p) => (
              <tr key={p.id}><td><Link to={`/p/${p.slug}`}>{p.title}</Link></td><td>{p.status}</td><td>{p.featured ? `#${p.featuredOrder}` : '—'}</td></tr>
            ))}
          </tbody>
        </table>
      </div>
